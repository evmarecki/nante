...
for i in range(0, len(MovieTitles)-1):
    s= "INSERT INTO Movies (city, Theater_name ,title, time_and_day, length, rating, genre) VALUES ('Accra', 'Silver Bird Cinemas',%s, %s, %s, %s, %s);"
    cur.execute(s, MovieTitles[i], MovieTimes[i], MovieLength[i], MovieRating[i], MovieGenre[i])
